...
class AgendamentoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['servico'].queryset = Servico.objects.filter(ativo=True).order_by('categoria', 'nome')
    ...
    def clean(self):
        ...
        if data and hora:
            ...
            if profissional:
                ...
                if not (profissional.horario_inicio <= hora <= profissional.horario_fim):
                    raise forms.ValidationError(
...
class ProfissionalForm(forms.ModelForm):
    ...
    class Meta:
        model = Profissional
        fields = ['nome', 'telefone', 'email', 'especialidades', 'horario_inicio', 'horario_fim']
        widgets = {
            'nome': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'Digite o nome completo'
...
class ServicoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['duracao_minutos'].help_text = 'Duração padrão: 60 minutos (não editável)'
