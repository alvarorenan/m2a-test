...
class RelatorioService:
    ...
    @staticmethod
    def relatorio_servicos_concluidos(data_inicio=None, data_fim=None, profissional_id=None):
        ...
        if stats_gerais['total_agendamentos'] > 0:
            stats_gerais['ticket_medio'] = stats_gerais['receita_total'] / stats_gerais['total_agendamentos']
        else:
    ...
    def dashboard_stats(data=None):
        ...
        stats = {
            ...
            'agendamentos_confirmados': agendamentos_dia.filter(status='CONFIRMADO').count(),
            'agendamentos_concluidos': agendamentos_dia.filter(status='CONCLUIDO').count(),
            'agendamentos_cancelados': agendamentos_dia.filter(status='CANCELADO').count(),
        }
    ...
    def relatorio_profissional(profissional_id, periodo_dias=30):
        ...
        servicos_realizados = concluidos.values(
            'servico__nome'
        ).annotate(
            quantidade=Count('id')
        ...
        return {
            ...
            'taxa_conclusao': (concluidos.count() / total_agendamentos * 100) if total_agendamentos > 0 else 0,
        }