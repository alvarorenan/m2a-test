...
class ClienteAdmin(admin.ModelAdmin):
    list_display = ['nome', 'telefone', 'email', 'ativo', 'total_agendamentos', 'data_cadastro']
...
class ServicoAdmin(admin.ModelAdmin):
    list_display = ['nome', 'categoria', 'preco_formatado', 'duracao_formatada', 'ativo', 'total_agendamentos']
...
class ProfissionalAdmin(admin.ModelAdmin):
    list_display = ['nome', 'telefone', 'horario_trabalho', 'ativo', 'total_agendamentos']
...
class AgendamentoAdmin(admin.ModelAdmin):
    list_display = [
        'cliente', 'profissional', 'servico', 'data_hora_formatada', 
        'status_colorido', 'preco_final_formatado'
    ]
    list_filter = [
        'status', 'data_hora', 'profissional', 'servico__categoria',
        'data_cadastro'
    ]
    search_fields = [
        'cliente__nome', 'profissional__nome', 'servico__nome',
        'observacoes'
    ]
    ...
    fieldsets = (
        ...
        ('Detalhes', {
            'fields': ('status', 'preco_final', 'observacoes')
        }),
        ('Informações Calculadas', {
            'fields': ('data_hora_fim_calculada',),
            'classes': ('collapse',)
        }),
        ('Auditoria', {
            'fields': ('data_cadastro', 'data_atualizacao'),
    ...
    actions = ['marcar_como_confirmado', 'marcar_como_concluido', 'marcar_como_cancelado']
    
    def marcar_como_confirmado(self, request, queryset):
        updated = queryset.filter(status='AGENDADO').update(status='CONFIRMADO')
        self.message_user(
            request,
            f'{updated} agendamento(s) marcado(s) como confirmado(s).'
        )
    ...
    def marcar_como_concluido(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} agendamento(s) marcado(s) como concluído(s).'
        )
    ...
    def marcar_como_cancelado(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} agendamento(s) cancelado(s).'
        )
